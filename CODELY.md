...
- [2026-08-07 11:50:50] 2026-08-07 预存测试失败（与本批修复无关）：全量 `vitest run` 有 ~67 项失败，均为预存问题——25 个组件测试文件中 `wrapper.emitted()` 返回 undefined（Vue Test Utils 版本兼容问题）+ `test/config/version.test.ts` 报 `No such built-in module: node:`。战斗+敌人模块测试（1631 项，53 文件，含 P3-187 新增 85 用例）全通过。排查时可直接忽略这些文件级失败。
