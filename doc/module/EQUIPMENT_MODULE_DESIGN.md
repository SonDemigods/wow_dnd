...
| 更新说明 | 严格对齐源码：移除不存在的 `IEquipmentService` 接口（Store 即对外接口）；移除不存在的 `getRarityConfig`/`getRarityColor`/`syncStatsToCharacter` 方法；移除错误的 `RARITY_CONFIG` 引用说明（装备模块未引用此常量）；新增 Phase 5.3 套装系统类型定义（`SetBonus`/`SetBonusEffect`/`ItemSet`）；补全 `EquipmentItem` 的 `classRestriction`/`setId` 字段；新增 `EquipmentStorage` 导入/导出类型；新增 `EquipmentTemplateStorage` 完整类型定义；补全 Service 层纯函数（`ALL_EQUIPMENT_SLOTS`/`createEmptySlotMap`/`checkClassRestriction`/`countSetPieces`/`getActiveSetBonuses`/`getSetPieceCount`）；标注 `isSlotOccupied` 为内部函数；补全 Store 导出（`equipmentTemplates`/`activeSetBonuses`/`slotList`/`weaponSlots`/`armorSlots`/`addEquipmentTemplate`/`removeEquipmentTemplate`/`getEquipmentTemplate`）；新增 A1/G1 回调注入机制（`setInventoryCallbacks`/`clearInventoryCallbacks`）；新增 BIZ-13 套装奖励重新应用逻辑；修正文件结构树为标准树形格式 |
...
| 文件 | 职责 |
|------|------|
| `index.ts` | 模块入口，统一导出 types（10 项）、`EquipmentDbService` 类与 `equipmentDbService` 实例、`useEquipmentStore`、`setInventoryCallbacks`、`clearInventoryCallbacks` |
...
| `store.ts` | Pinia Store（`defineStore('equipment')`），装备状态唯一持有者，编排 Service 纯函数 → 更新状态 → 同步角色属性 → 持久化 → 记录日志；含回调注入机制（A1/G1）和套装奖励重新应用逻辑（BIZ-13） |
| `service.ts` | 纯函数服务层（无状态、无副作用），负责槽位校验、属性计算、可装备性检查、套装统计等核心业务逻辑 |
...
/** 装备数据行存储接口（char_equipment 表，仅存装备 ID 映射） */
...
> 回调由 `GameBootstrap.initialize` 在 inventory 初始化后注入，`dispose` 时清除。回调未注入时 `equipItem` / `doUnequip` 会输出 `console.warn` 并失败/不放回背包。
...
5. **从背包移除**（A1/G1 回调）：`inventoryRemoveItemCallback(item.id, 1)`，移除数量 ≤ 0 → 返回 false
6. **卸下旧装备**：`doUnequip(slot)`（try/catch 含回滚，卸下失败时通过 `inventoryAddItemCallback` 放回背包）
7. **装备新物品**：写入 `equipment.value[slot] = { item, equippedAt: Date.now() }`
8. **应用属性加成**：`characterStore.applyBonus(computeEquipBonus(item))`（bonus 非空时）
9. **重新应用套装效果**（BIZ-13）：`reapplySetBonuses()` 计算套装奖励 diff 并同步
10. **持久化**：`persist()` 写入 `char_equipment` 表
11. **记录冒险日志**：`useLogStore().addLogEntry()`
...
4. **持久化**：`persist()`
5. **记录冒险日志**：`useLogStore().addLogEntry()`

### doUnequip 内部流程（被 equipItem 和 unequipItem 复用）

1. 获取槽位装备，空槽位返回 null
2. `removeBonusesFromSlot(slot)`：计算并移除装备 bonus
3. `equipment.value[slot] = null`
4. `inventoryAddItemCallback(equippedItem.item.id, 1)`：放回背包（回调未注入时 `console.warn`）
5. 返回卸下的 `EquippedItem`

### 属性计算与同步流程

1. `totalStats`（computed）：遍历所有槽位的 `equippedItem.item.bonus`，按属性键累加，返回完整 `Stats` 对象
...
> 在 `equipItem`、`unequipItem`、`initialize`、`reset` 中调用，确保装备变化后套装奖励正确同步。

### 初始化流程（initialize）

1. 设置 `isLoading = true`，记录 `currentCharacterId`
2. 加载所有装备模板到 `equipmentTemplates` Map（`getAllEquipmentTemplates()`）
...
3. **BIZ-13**：调用 `reapplySetBonuses()` 移除所有套装效果
...
套装系统通过 `setId` 字段将装备归组，穿戴达到指定件数时激活对应奖励。套装定义存储在 `src/data/config_item_sets.ts` 的 `ITEM_SETS` 中。

### 套装奖励触发条件

- 装备的 `setId` 字段标识所属套装
- `countSetPieces(equipment)` 统计各套装的穿戴件数
- `getActiveSetBonuses(equipment)` 筛选 `requiredPieces <= 当前穿戴件数` 的奖励
...
| 数据库表 | Key | 数据结构 | 说明 |
|----------|-----|----------|------|
| `char_equipment` | `characterId` | `EquipmentDataStorage` | 角色装备 ID 映射（仅存 ID，按角色隔离） |
| `config_equipmentItems` | `id` | `EquipmentTemplateStorage` | 装备模板完整定义 |
...
| `equipmentTemplates` | ref<Map<string, EquipmentItem\>\> | 装备模板缓存（从 `config_equipmentItems` 加载） |
...
| 名称 | 类型 | 说明 |
|------|------|------|
| `totalStats` | computed<Stats\> | 当前装备提供的总属性加成（遍历所有槽位 bonus 累加） |
...
| `equipItem` | `(slot: EquipmentSlot, item: EquipmentItem) => Promise<boolean>` | 装备物品到指定槽位（8 步流程） |
...
| `addEquipmentTemplate` | `(item: EquipmentItem) => void` | 添加装备模板（同步缓存与 DB） |
...
| 名称 | 类型 | 说明 |
|------|------|------|
| `SLOT_CONFIG` | Record<EquipmentSlot, {name, icon}\> | 槽位 UI 展示配置（名称和图标） |
| `setInventoryCallbacks` | function | 设置背包操作回调（A1/G1） |
| `clearInventoryCallbacks` | function | 清除背包操作回调 |
...
| 背包模块 | 回调注入（A1/G1） | `inventoryAddItemCallback(itemId, quantity)` 放回装备，`inventoryRemoveItemCallback(itemId, quantity)` 移除装备 |
| 日志模块 | 直接 Action 调用 | `addLogEntry()` 记录装备/卸下操作的冒险日志 |
| 套装配置 | 静态 import | `ITEM_SETS` 套装定义数据 |
...
| 回调未注入 | `inventoryRemoveItemCallback === null` | `console.warn` 提示检查 GameBootstrap，`equipItem` 返回 false |
| 背包移除失败 | `inventoryRemoveItemCallback` 返回 ≤ 0 | `equipItem` 返回 false |
| 卸下旧装备异常 | `doUnequip` 抛出错误 | catch 块回滚：通过 `inventoryAddItemCallback` 放回已移除的装备 |
| 回调未注入（卸下） | `inventoryAddItemCallback === null` | `console.warn` 提示，装备未放回背包（生产环境不应出现） |
...
| 模板内存缓存 | `equipmentTemplates` Map 缓存所有装备模板 | 快速访问，避免重复 DB 查询 |
...
| 类型收窄 | `mapTemplateToEquipmentItem` 集中处理 DB string → 运行时字面量联合类型的转换 |
| 异常捕获 | `dbService.withRetry` 含重试机制 |
| 数据净化 | 写入前通过 `toRawData()` 处理数据（去除 undefined） |
| 浅拷贝返回 | `getEquipment` / `getEquippedItem` 返回浅拷贝，避免外部修改内部状态 |
| 装备失败回滚 | `equipItem` 卸下旧装备异常时，通过回调将已移除的装备放回背包 |
| 回调泄漏防护 | `clearInventoryCallbacks` 在 dispose 时清除回调引用 |
