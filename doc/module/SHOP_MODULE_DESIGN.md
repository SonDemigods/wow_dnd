...
| 生成日期 | 2026年8月3日 |
| 所属模块 | `modules/shop` |
| 更新说明 | P3-116 全局状态收敛与持久化重构：`currentShopId` 由本地 `ref` 改为只读 `computed` 代理 `gameStore.currentShopId`，所有修改统一通过 `gameStore.setCurrentShopId()` 完成（触发持久化到 `runtime_gameState` 表 `id='gameState'`）；`shop/db.ts` 移除 `saveCurrentShopId`/`getCurrentShopId`（迁移至 `modules/game` 的 GameStore，经 `gameStateHelper` 读写）；`init` 流程改为校验 GameStore 已恢复的商店 ID 有效性；同步修正 `ShopConfig` 补充可选 `locationId` 字段（P3-134）；补充 P2-54 购买库存更新失败回滚、P1-15 任务物品不可出售机制；价格体系明确为 `service.ts` 私有 `getRarityMultiplier`（未使用 `config/inventory.ts` 的 `RARITY_PRICE_MULTIPLIER`/`RARITY_SELL_DISCOUNT`） |
...
商店模块是游戏经济系统的核心组件，负责管理商品的随机刷新、购买和出售功能。采用"当前商店"模式（同一时间只打开一个商店），商品列表按 shopId 隔离存储。回购列表持久化到 IndexedDB，支持页面刷新后恢复。当前商店 ID（`currentShopId`）自 P3-116 起收敛至 GameStore 统一持有并持久化，本模块通过只读 computed 代理访问，不再直接读写 `runtime_gameState` 表。
...
| 数据持久化 | 商店配置、商品、回购列表由本模块存储；当前商店 ID 由 GameStore 收敛持久化（P3-116，`runtime_gameState` 表 `id='gameState'`） |
...
- 冒险日志模块: 记录交易事件日志
- 游戏模块: 当前商店 ID 读写与持久化（`gameStore.setCurrentShopId/getCurrentShopId`，P3-116 收敛）
...
- **商店模块 → 游戏模块**：打开/关闭商店时通过 `gameStore.setCurrentShopId()` 持久化当前商店 ID（`currentShopId` 为只读 computed 代理，P3-116）
...
| FR-SHOP-011 | 数据持久化存储（配置、商品、回购列表；当前商店 ID 由 GameStore 持久化，P3-116） | 存档系统 |
| FR-SHOP-012 | 页面刷新后恢复上次打开的商店 ID（GameStore.initialize 从 DB 恢复）和回购列表 | 用户体验 |

### 非功能需求

| 需求编号 | 需求描述 | 优先级 |
|----------|----------|--------|
| NFR-SHOP-001 | 交易操作失败时回滚数据（金币返还、库存更新失败回滚，P2-54） | 高 |
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `init` | `() => Promise<void>` | 初始化商店模块（加载配置、校验 GameStore 恢复的商店 ID、恢复回购列表和 lastRefresh） |
| `openShop` | `(shopId: string) => Promise<void>` | 打开指定商店（加载/生成商品、检查刷新、通过 `gameStore.setCurrentShopId` 持久化商店 ID） |
...
| `closeShop` | `() => Promise<void>` | 关闭当前商店（通过 `gameStore.setCurrentShopId(null)` 清空、清空状态） |
...
| `currentShopId` | `ComputedRef<string \| null>` | 当前打开的商店 ID（只读 computed 代理 `gameStore.currentShopId`；修改必须经 `gameStore.setCurrentShopId()` 完成以触发持久化，P3-116） |
...
export interface ShopConfig {
  ...
  /** 商店所属地点 ID（可选），用于关联商店与地图地点（P3-134） */
  locationId?: string;
}
...
### 种子商店清单（SHOPS，来自 `config_shops`）

| 商店 ID | 名称 | 类型 | 刷新间隔（毫秒） |
|---------|------|------|------------------|
| `general_goods` | 杂货铺 | general | 300000（5 分钟） |
| `potion_shop` | 炼金术士小屋 | potion | 600000（10 分钟） |
| `scroll_shop` | 奥术商店 | scroll | 900000（15 分钟） |
| `supply_shop` | 旅行者补给站 | general | 300000（5 分钟） |
| `alchemy_shop` | 药剂商行 | potion | 600000（10 分钟） |
| `mysterious_trader` | 旅行商人 | general | 1800000（30 分钟） |
...
3. **P3-116**：`currentShopId` 已由 `gameStore.initialize()` 在应用启动时从 DB 恢复（`App.vue` 调用，持久化于 `runtime_gameState` 表 `id='gameState'`）；此处仅校验其有效性——若保存在商店配置中不存在（数据损坏或版本变更），则通过 `gameStore.setCurrentShopId(null)` 清空
...
7. **P3-116**：通过 `gameStore.setCurrentShopId(shopId)` 持久化当前商店 ID（触发 GameStore 持久化，只读 computed 自动反映）
...
11. **P2-54**：若更新商店库存时 DB 写入异常，回滚背包（`inventoryStore.removeItem` + `flushPersist` 等待持久化完成）与金币（`characterStore.gainGold(totalPrice)`），Toast 提示，`errorReporter` 上报，返回 false
12. 调用 `mergeItems` 刷新 `currentItems`
13. 发射 `SHOP_TRANSACTION` 事件（携带 shopId, itemId, quantity, totalPrice）
14. 记录冒险日志
...
4. **P1-15**：任务物品（`type === 'quest'`）不可出售，直接返回 false（防止玩家出售后无法找回导致存档损坏）
5. 调用纯函数 `computeSellPrice(itemTemplate)` 计算回收单价，售价 ≤ 0 时返回 false
6. 调用 `inventoryStore.removeItem(itemId, quantity)` 从背包移除（返回实际移除数量）
7. 计算实际售价 = 单价 × 实际移除数量
8. 调用 `characterStore.gainGold(actualSellPrice)` 增加金币
9. 加入回购列表（`_replaceSoldItems` 整体替换，同物品多次出售合并数量）
10. **BIZ-16**：持久化回购列表到 `runtime_shopSoldItems` 表
11. 调用 `mergeItems` 刷新 `currentItems`（回购物品出现在商店顶部）
12. 发射 `SHOP_TRANSACTION` 事件（携带 shopId, itemId, quantity, sellPrice）
13. 记录冒险日志
...
2. **P3-116**：通过 `gameStore.setCurrentShopId(null)` 清空并持久化当前商店 ID（触发 GameStore 持久化，只读 computed 自动反映）
3. 清空 `currentItems`
4. 发射 `SHOP_CLOSED` 事件
...
| 价格类型 | 计算公式 |
|----------|----------|
| 买入价 | `Math.round(item.value × getRarityMultiplier(rarity))`（`service.ts` 私有函数） |
| 卖出价 | `Math.round(买入价 × 0.5)` |

稀有度倍率（`service.ts` 私有 `getRarityMultiplier`）：common=1.0, uncommon=2.0, rare=5.0, epic=10.0, legendary=20.0，未识别的稀有度回退 1.0。
说明：shop 模块价格计算依赖自身私有的 `getRarityMultiplier`，未使用 `config/inventory.ts` 的 `RARITY_PRICE_MULTIPLIER`/`RARITY_SELL_DISCOUNT`（后者为背包等模块的配置常量）。
...
| `runtime_gameState` | `gameState` | `GameRuntimeState` | 全局游戏状态（`currentCharacterId`/`currentShopId`/`lastPlayedAt`/`initializedAt`/`gameSettings`），由 GameStore 经 `gameStateHelper` 读写（P3-116） |
...
| `locationId` | string \| undefined | 商店所属地点 ID（可选，P3-134），用于关联商店与地图地点；种子数据未配置 |
...
商店配置（`config_shops`）、商品数据（`runtime_shopItems`）和回购列表（`runtime_shopSoldItems`）均为全局共享数据，不按角色隔离。当前打开的商店 ID 自 P3-116 起由 GameStore 收敛持有，经 `gameStateHelper` 持久化到 `runtime_gameState` 表（`id='gameState'`），本模块不再直接读写该表。
...
| `generateShopItems(shopConfig, allItems, rng?)` | 纯函数 | 根据商店配置生成商品列表（6-12 件，库存 1-5，BIZ-21 限购；可注入确定性 RNG） |

私有函数：`getRarityMultiplier(rarity)`（稀有度倍率，common=1.0/uncommon=2.0/rare=5.0/epic=10.0/legendary=20.0）、`getMaxPurchaseCount(rarity)`（BIZ-21 购买次数上限）。
...
| `getShopItems(shopId)` | 获取商品列表（仅 items，不存在时返回 null） |
...
### currentShopId 持久化说明（P3-116）

`saveCurrentShopId`/`getCurrentShopId` 已从本模块移除。`currentShopId` 的读写与持久化统一由 GameStore（`src/modules/game`）承担：通过 `gameStore.setCurrentShopId()/getCurrentShopId()` 访问，内部经 `gameStateHelper` 操作 `runtime_gameState` 表（`id='gameState'`）。本模块不再直接访问 `runtime_gameState` 表。
...
- **背包模块**: `useInventoryStore()` 的 `addItem`、`removeItem`、`getItemInfo`、`getAllItems`、`flushPersist`
- **日志模块**: `useLogStore()` 的 `addLogEntry`、`generateLogId()`
- **游戏模块**: `useGameStore`（来自 `@/modules/game`）的 `setCurrentShopId`、`getCurrentShopId`（P3-116：currentShopId 收敛与持久化）
- **Toast 通知**: `useToast`（来自 `@/composables/useToast`）
- **配置数据**: `SHOPS` 常量（来自 `@/data/config_shops`，硬编码种子回退）
- **数据层**: `dbService.withRetry`（来自 `../data/core`）
- **错误上报**: `errorHandler`（来自 `@/services/ErrorHandler`）、`errorReporter`（来自 `@/utils/errorReport`）
...
| 背包模块 | 调用 | 购买时 `addItem`、出售时 `removeItem`、回滚时 `flushPersist`、查询 `getItemInfo/getAllItems` |
| 探索模块 | 调用 | 探索中的【商店】打开商店（`openShop`） |
| 冒险日志模块 | 调用 | 记录交易事件 |
| 游戏模块 | 调用 | 当前商店 ID 读写与持久化（`setCurrentShopId/getCurrentShopId`，P3-116） |
...
| 库存更新失败 | 购买时 DB 写入异常 | 回滚背包（`removeItem` + `flushPersist`）与金币（`gainGold`）（P2-54），Toast 提示，`errorReporter` 上报，返回 false |
| 物品无法出售 | 物品模板不存在、售价 ≤ 0 或为任务物品（`type='quest'`，P1-15） | 返回 false |
...
| 交易原子性 | 先验证再扣金币，背包满时按比例返还金币（P2-1）；库存更新失败时回滚金币与背包（P2-54） |
...
| 全局状态收敛 | `currentShopId` 由 GameStore 统一持有并通过 `gameStateHelper` 事务性读-改-写持久化（P3-116） |
...
| 购买时库存更新失败 | P2-54 回滚背包与金币，Toast 提示，返回 false |
| 生成商品与回购物品 ID 冲突 | `mergeItems` 排除回购列表中已有的生成商品 |
| 出售任务物品 | P1-15 直接拒绝出售，返回 false |
...
| 商店 ID 数据损坏 | P3-116 `init` 校验 GameStore 恢复的 ID，不在配置中则通过 `setCurrentShopId(null)` 清空 |
...
src/modules/game/     # P3-116 新增：GameStore（全局状态收敛，currentShopId 由本模块持有）
  - store.ts          # useGameStore：currentShopId/currentCharacterId/gameSettings/lastPlayedAt/initializedAt
  - types.ts          # GameSettings、GameRuntimeState 类型定义
  - index.ts          # 模块入口，统一导出
```
...
| `db.ts` | IndexedDB 数据库操作层（`ShopDbService` 类 + `shopDbService` 单例），封装 `config_shops`、`runtime_shopItems`、`runtime_shopSoldItems` 表（P3-116：已移除 `gameState` 读写），含私有 `cloneShopConfig` 辅助 |
| `store.ts` | Pinia Store 状态管理（`useShopStore`），编排业务逻辑，跨模块调用 character/inventory/log/game Store 和 eventBus，含内部 `_replaceSoldItems`/`mergeItems`/`loadShopConfigs`/`loadOrGenerateItems`/`regenerateItems` 辅助函数；`currentShopId` 为只读 computed 代理 `gameStore.currentShopId`（P3-116） |
...
| `game/store.ts`（新增） | GameStore 持有并持久化 `currentShopId`（`setCurrentShopId`/`getCurrentShopId`），经 `gameStateHelper` 写入 `runtime_gameState` 表 `id='gameState'`（P3-116） |
...
| v6.0 | 2026-08-03 | P3-116 全局状态收敛与持久化重构：currentShopId 改为只读 computed 代理 GameStore，修改统一经 gameStore.setCurrentShopId()（触发持久化到 runtime_gameState 表 id='gameState'）；shop/db.ts 移除 saveCurrentShopId/getCurrentShopId（迁移至 modules/game）；init 流程改为校验 GameStore 恢复的商店 ID；修正 ShopConfig 补充可选 locationId（P3-134）；补充 P2-54 购买回滚、P1-15 任务物品不可出售机制；价格体系明确为 service.ts 私有 getRarityMultiplier；补充 SHOPS 种子商店清单与 modules/game 文件结构 | System |
