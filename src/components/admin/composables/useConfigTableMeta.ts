 ...
 *
 * 内部维护 5+ 张配置表的列/字段静态定义，以及字典值 → 中文翻译映射表。
 * 视图组件仅消费计算属性，不再持有任何业务定义。
 */
...
// ========== 共享类型定义（与 AdminTable.vue / AdminForm.vue 中定义结构兼容） ==========
// 注意：Vue 文件的命名类型导出无法从 .ts 文件直接消费（vue shim 限制），
// 故在此本地声明结构兼容的接口。若 AdminTable.vue / AdminForm.vue 中的定义变化，
// 需同步更新此处。

/** 单元格值类型（对齐 AdminTable.vue 中的 CellValue） */
type CellValue = string | number | boolean | null | unknown[];
...
/** 表单字段定义（对齐 AdminForm.vue 中的 FormField） */
interface FormField {
  key: string;
  label: string;
...
/** 显式导出本地类型，供 useConfigCrud 等同模块 composable 复用 */
...
/** 显式导出 AdminRecord 类型别名，便于 composable 消费方统一引用 */
